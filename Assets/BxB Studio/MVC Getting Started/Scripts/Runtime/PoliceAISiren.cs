...
namespace MVC.Demo
{
	public class PoliceAISiren : ToolkitBehaviourExtension<Vehicle>
	{
		...
		public void Restart()
		{
			...
			if (!Base || !Base.HasAI || !Base.ActiveAIBehaviour)
			{
				ToolkitDebug.Error($"The `{nameof(PoliceAISiren)}` component has to have a `{nameof(Vehicle)}` and/or a `{nameof(VehicleAIBehaviour)}` component attached to the same GameObject!", gameObject);

				return;
			}
			else if (SirenClips.Length < 1)
			{
				ToolkitDebug.Warning($"We couldn't start the `{nameof(PoliceAISiren)}` component for \"{name}\" because the `{nameof(SirenClips)}` list is empty!", gameObject);
