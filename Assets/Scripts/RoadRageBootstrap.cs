...
namespace RoadRage.UnityRemake
{
    public sealed class RoadRageBootstrap : MonoBehaviour
    {
        ...
        private void BuildLighting()
        {
            ...
            // brightness"; the moods were authored before the post pipeline existed.
			...
			zoneTonemap.mode.Override(mood.TonemapNeutral == 1 ? TonemappingMode.Neutral : TonemappingMode.ACES);
			...
			zoneGrading.postExposure.Override(mood.PostExposure + weather.ExposureAdd + GlobalExposureLift);
			zoneGrading.contrast.Override(mood.Contrast != 0f ? mood.Contrast : 6f);
			zoneGrading.saturation.Override(
				(mood.Saturation != 0f ? mood.Saturation : -2f) + GlobalSaturationOffset);
        ...
        private struct BiomeMood
        {
            ...
            /// URP ColorAdjustments saturation (-100 to 100). 0 means use default (-2).
            public float Saturation;
            /// URP ColorAdjustments contrast (-100 to 100). 0 means use default (6).
            public float Contrast;
            /// 0 = ACES (default), 1 = Neutral (desaturated biomes like Manhattan).
            public int TonemapNeutral;
        }
        ...
        private BiomeMood Mood(int biomeIndex) => biomeIndex switch
        {
            ...
            8 => new BiomeMood // MANHATTAN — overcast hazy NYC daylight, desaturated concrete tones
            {
        ...
        private void TryStageHitAndRun(float speedKph)
        {
            ...
            foreach (var traffic in TrafficCarController.Active)
            {
                ...
                foreach (var other in TrafficCarController.Active)
                {
        ...
        private void BlendZoneLighting(float playerDistance)
        {
            ...
            if (zoneTonemap != null)
                zoneTonemap.mode.Override(here.TonemapNeutral == 1 ? TonemappingMode.Neutral : TonemappingMode.ACES);
        }
        ...
        private static BiomeMood LerpMood(BiomeMood a, BiomeMood b, float t) => new()
        {
            ...
            TonemapNeutral = t >= 0.5f ? b.TonemapNeutral : a.TonemapNeutral,
        };
        ...
        private void BuildCyberSprawl()
        {
            ...
            ScatterBand(18f, 13.5f, 14.5f, (d, l, s) =>
            {
                var tree = PlaceBiomeModelOnRoad("Buildings", "DemoCity/tree_1",
                    materials["City Palm"], d, l, 0.14f, new Vector3(0f, Random.Range(0f, 360f), 0f), Vector3.one, "Street Tree");
                if (tree != null) NormalizeModelHeight(tree, Random.Range(5f, 9f), 0.14f);
                return tree;
            });

            // NYC brownstone/apartment buildings only (building_1 through building_8 exist on disk)
            var nycBuildings = new[]
            {
            ...
            // DemoCity office towers — proper PBR highrise facades with windows
            var officeBuildings = new[]
            {
                "office_building_1", "office_building_2",
                "office_building_3", "office_building_4"
            };

            // Synthwave generic towers for deep skyline variety
            var synthTowers = new[]
            {
                "Buildings/SM_building_01", "Buildings/SM_building_02", "Buildings/SM_building_03",
                "Buildings/SM_building_04", "Buildings/SM_building_05", "Buildings/SM_building_06",
            ...
            for (var z = SegBegin(0f, 22f); z < segEnd; z += 22f)
            {
                ...
                for (var side = -1; side <= 1; side += 2)
                {
                    ...
                    // LAYER 1: Street-level storefronts & brownstones (≥20m from center, 18–38m tall)
                    ...
                    var front = PlaceBiomeModelOnRoad("Buildings", frontMesh,
                        materials["City Concrete"], frontDistance, side * Random.Range(20f, 24f), 0f,
                        new Vector3(0f, facing, 0f), Vector3.one, "NYC Street Frontage");
                    if (front != null)
                    {
                        NormalizeModelHeight(front, Random.Range(18f, 38f));
                        SnapToGround(front, frontDistance);
                        EnsureOutsideRoad(front, frontDistance, side);
                    }

                    // LAYER 2: Mid-rise office towers behind the frontage (28–42m from center, 42–85m tall)
                    var midDistance = z + Random.Range(-4f, 4f);
                    var useDemoCity = block % 3 != 0;
                    if (useDemoCity)
                    {
                        var officeMesh = officeBuildings[BlockHash(block, side * 13) % officeBuildings.Length];
                        var midrise = PlaceBiomeModelOnRoad("DemoCity", officeMesh,
                            materials["Demo Highrise"], midDistance, side * Random.Range(28f, 42f), 0f,
                            new Vector3(0f, facing, 0f), Vector3.one, "Manhattan Office Tower");
                        if (midrise != null)
                        {
                            NormalizeModelHeight(midrise, Random.Range(42f, 85f));
                            SnapToGround(midrise, midDistance);
                            EnsureOutsideRoad(midrise, midDistance, side);
                        }
                    }
                    else
                    {
                        var nycMesh = nycBuildings[BlockHash(block, side * 13) % nycBuildings.Length];
                        var midrise = PlaceBiomeModelOnRoad("Buildings", nycMesh,
                            materials["City Concrete"], midDistance, side * Random.Range(28f, 42f), 0f,
                            new Vector3(0f, facing, 0f), Vector3.one, "Manhattan Mid-Rise");
                        if (midrise != null)
                        {
                            NormalizeModelHeight(midrise, Random.Range(45f, 78f));
                            SnapToGround(midrise, midDistance);
                            EnsureOutsideRoad(midrise, midDistance, side);
                        }
                    }

                    // LAYER 3: Skyline skyscrapers deep behind (50–90m from center, 90–200m tall)
                    var towerDistance = z + Random.Range(-10f, 10f);
                    var useSynth = block % 2 == 0;
                    if (useSynth)
                    {
                        var synthMesh = synthTowers[BlockHash(block, side * 3) % synthTowers.Length];
                        var tower = PlaceBiomeModelOnRoad("Synthwave", synthMesh,
                            materials["City Skyline"], towerDistance, side * Random.Range(50f, 90f), 0f,
                            new Vector3(-90f, facing, 0f), Vector3.one, "Manhattan Skyscraper");
                        if (tower != null)
                        {
                            NormalizeModelHeight(tower, Random.Range(90f, 200f));
                            SnapToGround(tower, towerDistance);
                            EnsureOutsideRoad(tower, towerDistance, side);
                        }
                    }
                    else
                    {
                        var nycMesh = nycBuildings[BlockHash(block, side * 3) % nycBuildings.Length];
                        var tower = PlaceBiomeModelOnRoad("Buildings", nycMesh,
                            materials["City Skyline"], towerDistance, side * Random.Range(50f, 90f), 0f,
                            new Vector3(0f, facing, 0f), Vector3.one, "Manhattan Skyscraper");
                        if (tower != null)
                        {
                            NormalizeModelHeight(tower, Random.Range(90f, 180f));
                            SnapToGround(tower, towerDistance);
                            EnsureOutsideRoad(tower, towerDistance, side);
                        }
                    }

                    // LAYER 4: Distant horizon towers (100–160m from center, 120–260m tall)
                    if (block % 2 == 0)
                    {
